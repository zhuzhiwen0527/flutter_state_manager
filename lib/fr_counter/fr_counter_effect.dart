...
Effect<Fr_counterState> buildEffect() {
  return combineEffects(<Object, Effect<Fr_counterState>>{
    Fr_counterAction.incrementCounterAction: _onIncrementCounterAction,
  });
}
/*
...
*/
void _onIncrementCounterAction(Action action, Context<Fr_counterState> ctx) {
  print('effect 拦截 消息再次转发至reduce');
  int count = ctx.state.count + 1;
  ctx.dispatch(Fr_counterActionCreator.updateCount(count));
  //此处可以处理相应的逻辑
}
